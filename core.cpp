...
int testModel(int argc, char* argv[]) {
    ...
    if (document.IsObject()) {
        ...
        for (rapidjson::SizeType imagePairIndex = startIndex; imagePairIndex < endIndex; ++imagePairIndex) {
            ...
            const bool isRecognized = areDuplicatesUsingMoments(firstImageMoments, secondImageMoments, THRESHOLD); */

            const bool isRecognized = areDuplicatesUsingPHashes(firstImage, secondImage, THRESHOLD);
...
int getOptimalHammingThreshold(int argc, char* argv[]) {
    ...
    if (document.IsObject()) {
        ...
        localAnswer.reserve(imagesPerProcess * 5);
        ...
        for (rapidjson::SizeType imagePairIndex = startIndex; imagePairIndex < endIndex; ++imagePairIndex) {
            ...
            localAnswer += std::to_string(calculateHammingDistance(firstImage, secondImage));
            localAnswer += ",";
            localAnswer += imagePair["answers"][0]["answer"][0]["id"].GetString();
            if (imagePairIndex != endIndex - 1) {
                localAnswer += "\n";
            }
            ...
            if (worldRank == ROOT_RANK) {
                ...
                std::string progress = std::to_string(estimatedProgress);
                progress = std::string(progress.begin(), std::find(progress.begin(), progress.end(), '.'));
                std::cout << "\rFinding optimal hamming threshold... Progress: " << progress + "%" << std::flush;
            }
        }
        ...
        if (worldRank == ROOT_RANK) {
            ...
            parseCSV(totalAnswer, distances, labels);

            std::cout << "Optimal Hamming distance threshold: "
            << std::to_string(findOptimalThreshold(distances, labels, 23.0, 50.0, 0.1)) << "\n";
...
double findOptimalThreshold(const std::vector<double>& distances, const std::vector<int>& labels, double start, double end, double step) {
    double bestThreshold = start;
    double bestF1Score = 0.0;

    for (double threshold = start; threshold <= end; threshold += step) {
        int truePositives = 0;
        int falsePositives = 0;
        int falseNegatives = 0;

        for (size_t i = 0; i < distances.size(); ++i) {
            bool is_duplicate = distances[i] < threshold;
            if (is_duplicate && labels[i] == 1) {
                truePositives++;
            } else if (is_duplicate && labels[i] == 0) {
                falsePositives++;
            } else if (!is_duplicate && labels[i] == 1) {
                falseNegatives++;
            }
        }

        const double precision = static_cast<double>(truePositives) / static_cast<double>(truePositives + falsePositives);
        const double recall = static_cast<double>(truePositives) / static_cast<double>(truePositives + falseNegatives);
        const double f1Score = 2 * precision * recall / (precision + recall);
